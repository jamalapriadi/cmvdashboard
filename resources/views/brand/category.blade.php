    ...
    $(function(){
        ...
        function showData(){
            $('.datatable-colvis-basic').DataTable({
                ...
                autoWidth: true,
                destroy: true,
                ajax: "{{URL::to('brand/data/category')}}",
                columns: [
                    {data: 'DT_Row_Index', name: 'DT_Row_Index',title:'No.',width:'5%',searchable:false,'orderable':false},
                    {data: 'id_category', name: 'id_category',title:'ID Category',width:'15%'},
                    {data: 'name_category', name: 'name_category',title:'Category'}
                ],
                buttons: [
                'copy', 'excel', 'pdf'
                ],
            ...
            $(document).on("click","#tambah",function(){
                ...
                '<label class="col-lg-3 control-label text-semibold">Nama Category</label>'+
                '<div class="col-lg-8">'+
                '<input class="form-control" name="name_category" required>'+
                '</div>'+
                '</div>'+
                '</div>'+
            ...
            $(document).on("submit","#form",function(e){
                ...
                if($("#form")[0].checkValidity()) {
                    ...
                    e.preventDefault();
                    $.ajax({
                        url         : "{{URL::to('brand/data/category')}}",
                        type        : 'post',
                        data        : data,
                        dataType    : 'JSON',
                        contentType : false,
            ...
            $(document).on("click",".edit",function(){
                ...
                $.ajax({
                    url:"{{URL::to('brand/data/category')}}/"+kode,
                    type:"GET",
                    success:function(result){
                        ...
                        '<div id="pesan"></div>'+
                        '<div class="form-group">'+
                        '<label class="col-lg-3 control-label text-semibold">Agency</label>'+
                        '<div class="col-lg-8">'+
                        '<input class="form-control" name="name_category" value="'+result.name_category+'">'+
                        '</div>'+
                        '</div>'+
                        '</div>'+
                        '<div class="modal-footer">'+
            ...
            $(document).on("click","#import",function(){
                var sample="{{URL::to('brand/data/api/sample-category')}}";
                ...
                '<div class="modal-header bg-primary">'+
                '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'+
                '<h4 class="modal-title" id="myModalLabel">Upload New Category</h4>'+
                '</div>'+
                '<div class="modal-body">'+
                '<div id="pesan"></div>'+
            ...
            $(document).on("click","#editlist",function(){
                var sample="{{URL::to('brand/data/api/sample-categorylist')}}";
                ...
                '<div class="modal-header bg-primary">'+
                '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'+
                '<h4 class="modal-title" id="myModalLabel">Upload Edit Category</h4>'+
                '</div>'+
                '<div class="modal-body">'+
                '<div id="pesan"></div>'+
            ...
            $(document).on("submit","#formImport",function(e){
                ...
                if($("#formImport")[0].checkValidity()) {
                    ...
                    e.preventDefault();
                    $.ajax({
                        url         : "{{URL::to('brand/data/api/import-category')}}",
                        type        : 'post',
                        data        : data,
                        dataType    : 'JSON',
                        contentType : false,
