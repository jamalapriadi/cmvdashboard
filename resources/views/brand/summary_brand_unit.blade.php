                    ...
                    <div class="tab-content pl-3 pt-2" id="nav-tabContent">
                        <div class="tab-pane fade show active" id="highlight-tab1" role="tabpanel" aria-labelledby="nav-home-tab">
                            ...
                            <div class="card bg-light">
                                <div class="card-header">Related Brand</div>
                                <div class="card-body">
                                    <a class="btn btn-primary text-white" id="addbrand">
                                        <i class="icon-add"></i> Add Brand
                                    </a>
                                    <hr>
                                    
                                    <div id="showRelatedBrand"></div>
...
@section('js')
    ...
    <script>
        $(function(){
            ...
            function sosmed(){
                $.ajax({
                    url:"{{URL::to('brand/data/unit-sosmed-by-brand')}}/"+id,
                    data:"type=brand",
                    beforeSend:function(){
            ...
            $(document).on("click",".editsosmed",function(){
                ...
                $.ajax({
                    url:"{{URL::to('brand/data/brand-sosmed')}}/"+kode,
                    type:"GET",
                    data:"type=corporate",
                    beforeSend:function(){
                        el+='<div id="modal_default" class="modal fade" data-backdrop="static" data-keyboard="false">'+
            ...
            $(document).on("click","#addbrand",function(){
                ...
                $.ajax({
                    ...
                    beforeSend:function(){
                        el+='<div id="modal_default" class="modal fade" data-backdrop="static" data-keyboard="false">'+
                            '<div class="modal-dialog">'+
                                '<form id="formBrand" onsubmit="return false;" enctype="multipart/form-data" method="post" accept-charset="utf-8">'+
                                    '<div class="modal-content">'+
                                        '<div class="modal-header bg-primary">'+
                                            '<h5 class="modal-title" id="modal-title">Add New Brand Unit</h5>'+
                                            '<button type="button" class="close" data-dismiss="modal">&times;</button>'+
                                        '</div>'+
            ...
            $(document).on("submit","#formBrand",function(e){
                ...
                if($("#formBrand")[0].checkValidity()) {
                    ...
                    e.preventDefault();
                    $.ajax({
                        url			: "{{URL::to('brand/data/add-brand-sosmed')}}",
                        type		: 'post',
                        data		: data,
                        dataType	: 'JSON',
                        contentType	: false,
            ...
            function showListBrand(){
                $.ajax({
                    url:"{{URL::to('brand/data')}}/"+id+"/show-list-brand",
                    type:"GET",
                    beforeSend:function(){
                        $("#showRelatedBrand").empty().html("<div class='alert alert-info'>Please Wait. . .</div>");
                    },
                    success:function(result){
                        var el="";
                        el+="<table class='table table-striped' id='tableShowRelatedBrand'>"+
                            "<thead>"+
                                "<tr>"+
                                    "<th>No.</th>"+
                                    "<th>Brand</th>"+
                        ...
                        el+="</tbody>"+
                        "</table>";
                        $("#showRelatedBrand").empty().html(el);
                        $("#tableShowRelatedBrand").DataTable();
                    },
                    errors:function(){
            ...
            $(document).on("click",".hapusrelatedbrand",function(){
                var related=$(this).attr("kode");
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            url:"{{URL::to('brand/data/hapus-related-brand')}}/"+id,
                            type:"DELETE",
                            data:"_token={{ csrf_token() }}&related="+related,
                            success:function(result){
                                if(result.success=true){
                                    swal("Deleted!", result.pesan, "success");
                                    showListBrand();
