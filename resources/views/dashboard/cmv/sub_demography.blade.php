        ...
        $(function(){
            ...
            function showSector(){
                ...
                $('.datatable-colvis-basic').DataTable({
                    ...
                    destroy: true,
                    ajax: {
                    	url:"{{URL::to('cmv/data/sub-demography')}}",
                    	data:f
                    },
                    columns: [
                        //{data: 'no', name: 'no',title:'No.',searchable:false,width:'5%'},
                        {data: 'subdemo_id', name: 'subdemo_id',title:'SUBDEMOGRAPHY ID',width:'17%'},
                        {data: 'demo.demo_name', name: 'demo.demo_name',title:'DEMOGRAPHY',width:'17%'},
                        {data: 'subdemo_name', name: 'subdemo_name',title:'SUBDEMOGRAPHY NAME'},
                            ...
                            '<div class="modal-content">'+
                                ...
                                '<div class="modal-body">'+
                                    ...
                                    '<div class="form-group">'+
                                        '<label class="control-label text-semibold">SUBDEMOGRAPHY ID</label>'+
                                        '<input class="form-control" name="subdemo" id="subdemo" placeholder="SUB DEMOGRAPHY ID" required>'+
                                    '</div>'+
                                    '<div class="form-group">'+
                                        '<label class="control-label text-semibold">SUBDEMOGRAPHY NAME</label>'+
                                        '<input class="form-control" name="name" id="name" placeholder="SUB DEMOGRAPHY NAME" required>'+
                                    '</div>'+
                                '</div>'+
            ...
            $(document).on("click",".editdemo",function(){
                ...
                $.ajax({
                    url:"{{URL::to('cmv/data/sub-demography')}}/"+kode+"/edit",
                    type:"GET",
                        ...
                        el+='<div id="modal_default" class="modal fade" data-backdrop="static" data-keyboard="false">'+
                            '<div class="modal-dialog">'+
                                '<form class="form-horizontal" id="formUpdate" onsubmit="return false;" enctype="multipart/form-data">'+
                                    '<div class="modal-content">'+
                                        '<div class="modal-header bg-primary text-white">'+
                                            '<h5 class="modal-title" id="modal-title">Edit Sub Demography</h5>'+
                                            '<button type="button" class="close" data-dismiss="modal"></button>'+
                                        '</div>'+
                                        ...
                                        '<div class="modal-body">'+
                                            ...
                                            '<div class="form-group">'+
                                                '<label class="control-label text-semibold">SUBDEMOGRAPHY ID</label>'+
                                                '<input class="form-control" name="subdemo" id="subdemo" placeholder="DEMOGRAPHY ID" value="'+result.subdemo_id+'" required>'+
                                            '</div>'+

                                            '<div class="form-group">'+
                                                '<label class="control-label text-semibold">DEMOGRAPHY NAME</label>'+
                                                '<input class="form-control" name="name" id="name" placeholder="SECTOR NAME" value="'+result.subdemo_name+'" required>'+
                                            '</div>'+
                                        '</div>'+
            ...
            $(document).on("submit","#formUpdate",function(e){
                ...
                if($("#formUpdate")[0].checkValidity()) {
                    ...
                    e.preventDefault();
                    $.ajax({
                        url			: "{{URL::to('cmv/data/sub-demography')}}/"+kode,
                        type		: 'post',
                        data		: data,
                        dataType	: 'JSON',
                        contentType	: false,
            ...
            $(document).on("click","#import",function(){
                var sample="{{URL::to('cmv/data/sample-sub-demography')}}";
                var el="";
                el+='<div id="modal_default" class="modal fade" data-backdrop="static" data-keyboard="false">'+
                    '<div class="modal-dialog">'+
                        '<form id="formImport" class="form-horizontal" onsubmit="return false;" enctype="multipart/form-data" method="post" accept-charset="utf-8">'+
                            '<div class="modal-content">'+
                                '<div class="modal-header bg-primary text-white">'+
                                    '<h5 class="modal-title" id="modal-title">Import Data SUb Demograpy</h5>'+
                                    '<button type="button" class="close" data-dismiss="modal"></button>'+
                                '</div>'+
                                ...
                                '<div class="modal-body">'+
                                    ...
                                    '</div>'+
                                    "<p><small>You can download 'Format DB Sub Demography' if you want to upload data to make sure that your data will upload has appropriate with format</small> <a href='"+sample+"'><label class='label label-success'>Click to Download Format DB Sub Demography</label></a>"+
                                '</div>'+
            ...
            $(document).on("submit","#formImport",function(e){
                ...
                if($("#formImport")[0].checkValidity()) {
                    e.preventDefault();
                    $.ajax({
                        url         : "{{URL::to('cmv/data/import-sub-demography')}}",
                        type        : 'post',
                        data        : data,
                        dataType    : 'JSON',
                        contentType : false,
...
@section('content')
    <div class="page-header">
        ...
        <div class="btn-group pull-right" role="group">
            <a href="#" class="btn btn-primary btn-labeled heading-btn" id="addsector"><b><i class="icon-task"></i></b> Create Sub Demography</a>
            <div class="dropdown">
                ...
                <div class="dropdown-menu">
                    ...
                    <a class="dropdown-item" href="{{URL::to('cmv/data/export-demography')}}">Export Excel</a>
                </div>
            </div>
        </div>
        </div>
