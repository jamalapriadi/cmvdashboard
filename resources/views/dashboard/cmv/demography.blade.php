        ...
        $(function(){
            ...
            function showSector(){
                $('.datatable-colvis-basic').DataTable({
                    ...
                    autoWidth: true,
                    destroy: true,
                    ajax: "{{URL::to('cmv/data/demography')}}",
                    columns: [
                        //{data: 'no', name: 'no',title:'No.',searchable:false,width:'5%'},
                        {data: 'demo_id', name: 'demo_id',title:'DEMOGRAPHY ID',width:'17%'},
                        {data: 'demo_name', name: 'demo_name',title:'DEMOGRAPHY NAME'},
                        {data: 'jumlahsub', name: 'jumlahsub',title:'SUBDEMOGRAPHY'},
            ...
            $(document).on("submit","#form",function(e){
                ...
                if($("#form")[0].checkValidity()) {
                    ...
                    e.preventDefault();
                    $.ajax({
                        url         : "{{URL::to('cmv/data/demography')}}",
                        type        : 'post',
                        data        : data,
                        dataType    : 'JSON',
                        contentType : false,
            ...
            $(document).on("click",".editdemo",function(){
                ...
                $.ajax({
                    url:"{{URL::to('cmv/data/demography')}}/"+kode+"/edit",
                    type:"GET",
            ...
            $(document).on("submit","#formUpdate",function(e){
                ...
                if($("#formUpdate")[0].checkValidity()) {
                    ...
                    e.preventDefault();
                    $.ajax({
                        url			: "{{URL::to('cmv/data/demography')}}/"+kode,
                        type		: 'post',
                        data		: data,
                        dataType	: 'JSON',
                        contentType	: false,
            ...
            $(document).on("click","#import",function(){
                var sample="{{URL::to('cmv/data/sample-demography')}}";
                var el="";
                el+='<div id="modal_default" class="modal fade" data-backdrop="static" data-keyboard="false">'+
                    '<div class="modal-dialog">'+
                        '<form id="formImport" class="form-horizontal" onsubmit="return false;" enctype="multipart/form-data" method="post" accept-charset="utf-8">'+
                            '<div class="modal-content">'+
                                ...
                                '<div class="modal-body">'+
                                    ...
                                    '</div>'+
                                    "<p><small>You can download 'Format DB Demography' if you want to upload data to make sure that your data will upload has appropriate with format</small> <a href='"+sample+"'><label class='label label-success'>Click to Download Format DB Demography</label></a>"+
                                '</div>'+
            ...
            $(document).on("submit","#formImport",function(e){
                ...
                if($("#formImport")[0].checkValidity()) {
                    e.preventDefault();
                    $.ajax({
                        url         : "{{URL::to('cmv/data/import-demography')}}",
                        type        : 'post',
                        data        : data,
                        dataType    : 'JSON',
                        contentType : false,
...
@section('content')
    <div class="page-header">
        ...
        <div class="btn-group pull-right" role="group">
            <a href="#" class="btn btn-primary btn-labeled heading-btn" id="addsector"><b><i class="icon-task"></i></b> Create Sector</a>
            <div class="dropdown">
